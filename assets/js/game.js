...
class Miner {
  ...
  draw() {
    ...
    const shakeOffsetX = mining
      ? Math.random() * shakeIntensity * 2 - shakeIntensity
      : 0;
    const shakeOffsetY = mining
      ? Math.random() * shakeIntensity * 2 - shakeIntensity
      : 0;
...
class Block {
  constructor(x, y, type) {
    ...
    this.type = type;
    this.img = new Image();
    this.img.src = `/assets/images/${this.type}_texture.png`;
  }
...
function gameLoop() {
  ...
  if (gameStarted) {
    ...
    ctx.fillText(
      "Press 'i' to open inv",
      canvas.width - 180 + camera.x,
      canvas.height - 20 + camera.y
    );
...
document.addEventListener("keydown", (e) => {
  ...
  switch (e.key) {
    case "ArrowUp":
      if (
        miner.y > 0 &&
        !isCollidingWithSolidBlock(miner.x, miner.y - blockSize)
      ) {
        miner.y -= blockSize;
      }
      break;
    ...
    case "ArrowLeft":
      if (
        miner.x > 0 &&
        !isCollidingWithSolidBlock(miner.x - blockSize, miner.y)
      ) {
        miner.x -= blockSize;
      } else {
