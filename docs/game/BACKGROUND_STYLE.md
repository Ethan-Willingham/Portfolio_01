...
## 15. Weather — clouds, precipitation, storms (v24.16)
...
**Clouds — the centrepiece.** 2–3 parallax billow-fbm cloud bands, built like the rest of the background (§6, §11): the heavy noise bake is cached per layer (rebuilt on a coverage bucket, one layer per frame so there is no hitch), and the per-frame cost is a few smoothed `drawImage` blits with a wind-driven horizontal drift offset. Drawn screen-locked inside `drawNightSkyToScreen`, clipped to the sky, OVER the stars + sun/moon (clouds occlude them) and BEHIND the parallax mountains.
- **Three-stage bake → shade → recolour (rebuilt v24.36 after the v1 clouds read as a blurry haze).** (1) The raw warped-billow *field* is baked ONCE per layer at decent resolution (the v1 mistake: a 200px tile blown up ~10x = a blurry smear). It is coverage-independent, so a weather change never re-bakes it. (2) The *shade* pass derives density + baked luminance, and is what makes it read as clouds, not noise: the threshold is HIGH so only the field's PEAKS become cloud — discrete masses with real clear-sky GAPS, not a flat sheet (the field mean is ~0.64, so a low threshold fills everything → haze); the band envelope biases the *threshold* (clouds thin toward the band edges) rather than dimming density, so cloud cores stay fully OPAQUE; and a short top-light march accumulates density straight up for exponential self-shadow → bright sunlit crowns fading to a shaded underside (the volume cue), plus a silver rim on thin edges. (3) The *recolour* lerps `colour = lerp(shadow, highlight, luminance)`, driven by **sun elevation + `dayW`**: highlight = white high → warm gold near the horizon → moon-silver at night; shadow = cool grey → dusky violet near the horizon → deep blue at night; both pull to flat slate in a storm. Shadow is kept darker/greyer than the sky so the cloud FORM reads (the v1 sky-tinted shadow washed out). Night/storm ends use the anchors `SKY.cloudSunHi / cloudMoonHi / cloudNightBase / cloudStormHi / cloudStormBase`.
- **Atmospheric perspective + parallax (§3/§8).** Far bands sit higher, drift slowest, lowest contrast; near bands sit lower toward the horizon, drift fastest, most contrast. Clouds fade out only at a deliberate high climb (`CLOUD_ALT_FULL`/`CLOUD_ALT_GONE`, decoupled from the sky-biome transition so ordinary surface flight never fades them).
- **The band is a FIXED screen rectangle (`cloudTop`/`cloudHeight` fractions of canvas height), never sized from the horizon.** Load-bearing: the v1 version sized the band to `min(ch, skyBottomPx)`, so flying — which moves the on-screen horizon — vertically *stretched* the clouds (they visibly expanded/contracted). Pinning the band to the screen like the sun/moon (celestials-pinned, §7) keeps cloud size constant at any altitude. `cloudHeight` defaults to **0.5** (≤ the ~0.55 surface-horizon fraction) so the band sits fully ABOVE the horizon while at/above the surface and the sky clip never bites it during flight; the clip only trims it once you descend underground. Do not re-tie cloud size to `skyBottomPx`.
