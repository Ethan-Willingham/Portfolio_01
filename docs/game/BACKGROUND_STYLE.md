...
## 15. Weather: clouds, precipitation, storms

Code: `js/sluice/155-weather.js`; colour anchors in `SKY` in 170; live controls in TUNING.md section 5.4. Dev **N** cycles moods. The mood machine eases coverage, darkness, precipitation and wind through clear, fair, cloudy, overcast, precip and storm.

**Cloud direction, rebuilt September 2026.** Broad, softly lit cloud banks with open sky between them. A cloud has a continuous body, a gently broken condensation base, smaller billows along its crown, and quiet interior detail. Bright contour bands, holes punched through opaque bodies, repeated companion stamps and scattered miniature copies are retired.

- **Geometry.** Three families (cirrus, cumulus, large cumulus), eight deterministic variants each. Overlapping ellipsoid depth fields join with a smooth maximum before shading. Smaller billows grow from their shoulders. Low-amplitude noise erodes the surface; density feathers the edges and controls opacity. Transparent gutters on all four sides prevent clipped sprite boundaries.
- **Lighting.** Surface slopes and height supply diffuse illumination. A faint transmission term lifts the crown without outlining every lobe. The geometry bake writes light and density maps; separate colour passes supply warm cream daylight, peach sunset, violet shade, subdued moonlight and storm slate. New colours belong in `SKY.cloud*`. Sprites are not mirrored: that would reverse the baked light direction.
- **Composition.** A deterministic 2D field places larger banks at continuous world altitudes, with correlated density and a wide size range. No extra small-cloud or companion passes. Horizontal camera parallax and integrated wind remain continuous. Vertical position tracks world altitude, and sizes stay fixed as the player flies through the weather. Cirrus takes over aloft and thins toward space.
- **Overcast.** One continuous stratus veil fades in at high coverage behind the banks. It fades away as the player climbs above the weather. Precipitation and lightning use the existing mood state.
- **Caching.** One geometry bake per frame, with a rotating cursor so repeated morph invalidations cannot starve the large-cloud family. Lighting changes recolour up to four cached sprites per frame. Highlight, shadow and contrast changes also invalidate colour immediately, including with the sun paused. Coverage, wind and camera motion do not rebuild geometry.

Clouds draw after the stars and celestial bodies, inside the sky clip and behind the mountains. Smooth upscaling is the existing deliberate exception to the pixel rules: it matches the atmospheric sky. Keep their interior contrast below the foreground and preserve the station and rig silhouettes.

**Acceptance checks.** Inspect clear, fair, cloudy, storm, midday, dusk and night at the surface; inspect cumulus and cirrus while climbing, then clear sky above the weather. Check mobile framing, transparent sprite borders, opacity in cumulus cores, repeated morph changes, live colour controls with a frozen sun, and steady frame cost after the cache warms. Screenshot files belong outside the repository.
