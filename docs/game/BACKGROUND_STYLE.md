...
## 15. Weather — clouds, precipitation, storms (v24.16; clouds rebuilt as an instanced cumulus field v26.21, altitude made continuous v26.38)
...
- **The sprite pool (LOBED silhouettes v26.39 — the slab era read as rectangles).** 3 appearance classes (`CLOUD_CLASSES`: wispy anisotropic cirrus, mid cumulus, big cumulus) × 8 seeded variants = 24 baked sprites, each ONE cloud. A cumulus silhouette is a MOUND of overlapping elliptical lobes (count, spread, radii, heights all hashed per sprite; one dominant tower, smaller flanks, capped below the tile top): the lobes give a convex lumpy outline, tapered rounded ends, and a base that exists only under the mass. The warped-billow field carves texture into that envelope, a cauliflower carve bites crown notches (fading at the base), then the top-light march (exponential self-shadow → bright crowns, shaded underside) + the silver rim. Cirrus streaks pinch to nothing at both ends over a wavy lumpy belly (the constant-width era read as long rectangles). Bakes amortise one sprite per frame (whole cast ~25 frames at boot); coverage changes NEVER re-bake — only `softness`/`rimGlow` moves or morphing do. Per-instance flips, scales, aspect squash, and ~45% two-stamp COMPOSITES (a second smaller variant beside the main mass, bases aligned) push variety combinatorial; instance cores stay near-opaque because translucent cumulus TERRACE into repeated arc seams where they overlap.
- **The field (how the sky is populated; altitude made CONTINUOUS v26.38).** The first instanced cut kept fixed-altitude lanes and the owner read them as rows of clouds, the same defect one level up. Now `CLOUD_ROWS` is one 2D hash lattice over (x, altitude), ~60 → ~4250 world px above the surface, with GEOMETRIC row sizing: short/narrow cells low (the sky is bottom-heavy and the resting view needs the candidates), tall/wide cells high. A slot (k, j) holds a cloud iff `hash(k)` clears the live coverage × an altitude-density curve, and the cloud's altitude is hashed across the FULL row height (rows abut, so altitude is continuous and nothing can line up); variant, x-jitter, scale, flip and fade come from more hashes — deterministic, infinite, non-repeating, zero storage, and coverage fades individual clouds in low-hash-first as weather changes. Heavy skies also fatten every instance slightly (the swell). v26.42: a large-scale CLUSTER noise (~4-cell wavelength, correlated across rows) modulates the fill so clouds bunch into banks with real gaps between groups, per-cell FRAGMENT shreds (up to two independent small stamps, alive slightly below the main threshold) scatter ragged bits between the masses, and the size jitter is small-biased (h^2) — an even spread of same-sized units reads as countable objects, not weather. Lobe layouts are remapped to always FIT the sprite tile and lobe heights scale collectively (per-lobe clamping flattened crowns into plateaus; overflow guillotined hard vertical edges). Every look property is a smooth function of the cloud's own altitude: the small → full-size progression runs INSIDE the resting view (tiny pale puffs at the ridge ~60-160, full cumulus by ~330, so no height reads as "the one size"), big+mid classes dither-blend low, cirrus takes over past ~1.55k, and density thins toward space. Horizontal MOTION (camera parallax + wind drift) is shared per row so cell enumeration stays consistent under any camera x and unbounded drift accumulation — ~10 motion planes read as smooth depth while POSITIONS stay continuous; the ridge row doubles as the far, slow valley-distance backdrop.
