...
| obstacle filter + feather | on | fixed | The obstacle texture is linearly filtered and its visible cutout fades across the edge. Smoke physics still uses the hard alpha threshold |
| `SMOKE_WATER_OBSTACLE` | `1` | 0/1 | v26.120: reconstruct the CPU water mirror onto world-anchored 4 px samples with a quadratic density kernel and continuous coverage. Replaces the old 8 px rectangle stamps whose density and speed switches exposed dark blue squares beneath smoke. Dense pools still block smoke; spray and fast falls remain passable, with a smooth 55 to 75 px/s transition. Cached float counts avoid saturation under compression. This changes only the smoke obstacle mask, not water rendering or physics. Bath and mobile exclusions remain. Never read the WebGPU water canvas here: before render it is cleared, and after render it causes a GPU sync stall |
...
| `LIQUID_SURF_BRIDGE` `edit²` | `0.9` | 0–2 | v26.57 gap bridge + contact wetting (composite edge only, no sim effect). A shallow sheet lives at the visibility threshold, so shimmer opened sub-threshold tears between particles and a flickering dark seam a few pixels above every floor. Near-threshold pixels now threshold on an effective field boosted by the strongest of eight neighbour taps; a tap landing inside solid terrain samples its REFLECTION (a Neumann mirror), which carries the field down to the contact line so resting water touches its floor. A gate zeroes the boost where the field is near zero: silhouettes do not swell and real spray stays separate. Foam, body and heat keep reading the raw field. Measured on a 10-row puddle: interior hole pixels 23 to 1-5, contact seam visually closed. 0 = the exact old edge. gm `water.SURF_BRIDGE` |
| `LIQUID_SURF_BRIDGE_R` `edit²` | `3` | 1–8 | v26.57 tap radius in device px for the bridge. Larger closes wider tears but rounds the silhouette more. Also sets the v26.58 contact reach (3 taps straight up = 3r). gm `water.SURF_BRIDGE_R` |
| (droplet coverage) | rides `SURF_BRIDGE` | | v26.60: the droplet pass's coverage test now evaluates the same BRIDGED effective field the composite paints (same taps, same gate), not the raw one. A calm surface's top particle row sits in the raw field's half-covered band, so every such particle wore a persistent light foam-tinted mark on top of already-painted body water (the owner's "light colored smaller particles" on calm ponds; measured through a settle window: pre-fix specks linger 6/5/6 per frame, fixed converges 2/1/0). Genuinely airborne spray has near-zero field, skips the bridged path by the same precondition, and keeps its marks unchanged |
| (contact invariant) | rides `SURF_BRIDGE` | | v26.58: the space between a body and the floor directly under it can never be air (the owner's cup argument: nothing could have let air in), yet a hard jiggle lifted the bottom particle row and flashed a dark line because mid-gap pixels carry near-zero field and the gated v26.57 taps never engaged there. A pixel with solid terrain just below and FULL BODY-STRENGTH field (>= `SURFACE_THRESH`) within `3 x SURF_BRIDGE_R` straight above now renders as water at that body's edge alpha, no own-field gate. A lone droplet peaks near half the threshold, so spray passing low cannot trigger it. Measured mid-slosh with alternating frames: levitation pixels 870-1490 per frame without it, ZERO with it (worst single-pixel flicker). **v26.58 also ends the Stage 5 boot flake at its root: the game's liquidStateTick now pushes sim params only after `liquidWGPU.ready` (the v26.53 contract: tests run at module defaults). Pushing mid-test raced the GPU dispatch against the reference's module-var reads; diffs sat at 90-99 percent of tolerance from v26.54 and failed world-dependently. Boot diffs are back to 0.001-0.004** |
...
The standalone control is **very calm to very lively** and opens at 42 percent,
the lower edge of the label `fluid`. Its quadratic calm response maps
`QUIET_VISC` 0.058 to 0.018,
...
| `LIQUID_TIMESCALE` `edit²` | `1.38366702` | v25.29 introduced the stable material clock rather than raising gravity back into the over-driven EOS regime. v26.53.1 retains the requested 42 percent `fluid` landing rate: `1.55 * (1 - 0.55 * 0.58^3)`. Per-substep v26.53 physics and splash trajectories remain bit-identical; the trajectory simply plays about 11 percent more gently than the historical 1.55 default. Wall-clock effective gravity is about 479 px/s², and awake water averages about 2.77 substeps per 60 Hz frame rather than 3.10. MAX_SUBSTEPS still caps slow frames. gm `water.TIMESCALE` (live, resets the bank); boot A/B `?wdbg=TIMESCALE:1.55` restores the historical rate |
...
**v26.69 scope change:** NPC bodies (`b.npc`, set only while the slime-NPC
brains are enabled; they SHIP OFF since v26.73) skip the dissolve entirely, same skip as the banya's `b.guest`
...
| Lever | Now | Range | Effect |
|---|---|---|---|
| `NPC` | `0` | 0/1 | Master (SHIPS OFF since v26.73, owner call): wild slimes think, wander, swim. Flipping to 0 sweeps intents/buoys/guest lanes once; `?npc=1` enables per boot |
...
The ascent horizon treatment (`js/sluice/158-horizon-atmos.js`, BACKGROUND_STYLE §16). Two systems share the `haze` gm group: the original aerial-perspective **veil** (`haze.*`, parked OFF since v24.53 because it fogged the whole visible ground) and the **horizon limb** (`limb.*`, ON). The limb continues the sky's below-line slice during ascent. It draws immediately after the sky, before mountains, the recessed bank, terrain, and entities. Those layers occlude it normally; the former post-world placement painted a straight stripe across the irregular bank skyline. The slice starts opaque and fades downward as its band height grows. The sky shader's separate horizon edge was fixed with a spherical primary ray in v24.138. Boot levers for screenshot harnesses: `?tod=0.755` freezes the sunset and `?alt=N` spawns the rig N world pixels above the surface. Pair with `?nosave=1`.
| Lever | Now | Range | Effect |
|---|---|---|---|
