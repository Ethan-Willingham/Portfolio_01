...
| `LIQUID_FLOOR_FRICTION` | `0.92` | 0.80–1.0 | Horizontal drag over solid floor — lower = more drag |
...
| `LIQUID_VISC_LIVE` | `0.05` | 0–0.6 | Grid viscosity while STIMULATED (v24.152: 0.08→0.05, near-raw while lively); blends to `LIQUID_GRID_VISC` as calm ramps. gm `water.VISC_LIVE` |
| `LIQUID_DAMP_LIVE` | `0.9985` | 0.99–1.0 | v24.152 THE SLOSH KNOB: per-substep damping while LIVELY (blends to `LIQUID_DAMPING` as calm ramps). gm `water.DAMP_LIVE`. **v24.157 SAFETY FLOOR: 0.9995 EXPLODED a fresh lake under sustained rig stim (the clamped-EOS pump compounds when dissipation ~0; the old stack was its bound). 0.9985 + MOTION 0.997 + VISC 0.10 hold `?pondtest=4` (permanent hard stim on a fresh fill) bounded — re-run that leg before raising any of the three** |
| `LIQUID_MOTION_LIVE` | `0.997` | — | Near-full APIC transfer while lively (blends to `LIQUID_WATER_MOTION_SCALE` 0.97 as calm ramps; 1.0 was part of the v24.156 explosion) |
...
| `LIQUID_CALM_MAX` | `0.5` | 0–1 | v25.39 — REST LIVELINESS CAP: the calm ramp parks here instead of 1.0, so the permanent rest state is the MID-SETTLE physics (half-strength brake, visc ~0.28, most particles above the sleep gate) — the owner's pick: fully-settled water "looks horrible... the transition from popcorny to still is right where I want it". 1 = the old dead-still settle (also re-arms the whole-body freeze latch, which needs calm ≥ 1 and is otherwise structurally disarmed); lower = livelier rest. gm `water.CALM_MAX` (live) |
