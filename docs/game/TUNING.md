...
| `LIQUID_TIMESCALE` `edit²` | `1.38366702` | v25.29 introduced the stable material clock rather than raising gravity back into the over-driven EOS regime. v26.55 sets the default to the standalone scale's 42 percent point, just inside `fluid`: `1.55 * (1 - 0.55 * 0.58^3)`. Per-substep physics and splash trajectories remain bit-identical; the trajectory simply plays about 11 percent more gently than the former 1.55 default. Wall-clock effective gravity is about 479 px/s², and awake water averages about 2.77 substeps per 60 Hz frame rather than 3.10. MAX_SUBSTEPS still caps slow frames. gm `water.TIMESCALE` (live, resets the bank); boot A/B `?wdbg=TIMESCALE:1.55` restores the former very-watery default |
