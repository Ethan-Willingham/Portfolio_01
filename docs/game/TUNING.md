...
| `SMOKE_WATER_OBSTACLE` | `1` | 0/1 | v25.47 — smoke COLLIDES WITH WATER: the water body is painted into the obstacle mask from the liquid CPU MIRROR (particle positions binned into 8-px cells, two alpha levels: full bins solid, rim/spray bins 0.4 = passable soft boundary), so smoke piles onto pond surfaces and curls around waterfalls instead of ghosting over them on a separate layer. Pure CPU geometry like the tile/jello passes (~0.3 ms per repaint; measured ZERO fps delta on/off, and the mirror covers the whole overscan domain). **LESSON — never drawImage the WebGPU water canvas for this: in the update phase it reads BLANK (the canvas texture is cleared after present; measured 0 px), and in the render phase the read is a GPU sync stall (measured 120 → 30 fps under camera motion). Both were shipped briefly in v25.46 and did nothing/hurt.** Mask refreshes every `SMOKE_OBST_WATER_EVERY` (8) frames while water exists + on the classic dirty triggers. Desktop canvas path only (the GL/mobile quad path skips water). Dev probe `window.__smokeObst` (canvas + info: paints/stamps/awakeT). gm `smoke.WATER_OBSTACLE`; 0 = the old separate-layer look |
