...
to interpolated skin edges. Mass-weighted position corrections exchange
momentum, and bounded friction grips the tracks. The recovering mesh supplies
...
While loaded, the rig and skin render the current contact step together.
Unloaded residents retain interpolation between ordinary physics ticks.
Verlet history is rescaled when contact requires a different step duration,
preserving velocity on entry and exit and on faster displays.
...
`/tmp`. `FPS`, `SPEEDS`, `CASES`, `DUMP`, and `PORT` narrow or relocate a run;
