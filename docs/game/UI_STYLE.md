...
> **Catalog levels (2026-07-18, v26.33):** the kit's catalog is a navigation STACK. Tabs hold the root list; `item.kind='folder'` rows descend on tap (chevron on the right), and `item.children` + `item.childLabel` give a normal item a secondary ghost button in the detail pane that pushes a deeper level (first shipped use: the drill's VIEW ALL 7 TIERS ladder, the §16.2 tier ladder finally built). While deep, the tab strip becomes a BACK chip + breadcrumb trail, content slides in sideways, Escape/B/ArrowLeft pop one level, the X still closes everything, and a purchase anywhere refreshes every level. Bespoke flows (future combine pickers) call `ukPush`/`ukPop` directly. See the 245 header comment for the full item spec.
...
Used for: hull (a GRID of armor tiles whose COUNT tracks the Hull Plating upgrade: 6 tiles at level 1, +3 per tier to 24 at level 7; the grid auto-sizes its tiles and rows to fit the bay), future shield panels.
