...
(function () {
  ...
  var TOY_VERSION = 'v4.36'; // shown in the engine stats; bump with the
  ...
  function waterLivelinessTick(dt) {
    // Ready-gated: hosts push live values only after the numbered boot
   ...
   * from the wall grid's horizontal runs, moving slime velocity boundaries,
   * and an idle gate so a dissipated field costs nothing. ==== */
  ...
  function toolTick(dt) {
    ...
    if (tool === 'draw' || tool === 'erase') {
      ...
      if (!grabBody && smokeActive && smokeAwakeT > 0 && (pvx !== 0 || pvy !== 0)) {
        SmokeFluid.splatVelocity(px / worldW, 1 - py / worldH,
