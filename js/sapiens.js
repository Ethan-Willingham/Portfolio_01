/* Sapiens: the five-imagined-orders gallery.
...
(function () {
  ...
  var orders = [].slice.call(source.querySelectorAll('.tale')).map(function (el) {
    return {
  ...
  var chips = orders.map(function (d, i) {
  ...
  function render() {
    ...
    prev.disabled = cur === 0;
    next.disabled = cur === orders.length - 1;
  ...
  function go(i) {
    cur = Math.max(0, Math.min(orders.length - 1, i));
    render();
  }
