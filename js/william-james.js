/* The Varieties of Religious Experience: the four-marks-of-mysticism gallery.
   Reads the authored .tale cards in the page, builds a one-at-a-time
   gallery with mark chips, arrow buttons, and keyboard support.
...
(function () {
  ...
  var marks = [].slice.call(source.querySelectorAll('.tale')).map(function (el) {
    return {
  ...
  function go(i) {
    cur = Math.max(0, Math.min(marks.length - 1, i));
    render();
  }
