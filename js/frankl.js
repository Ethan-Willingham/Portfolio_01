/* Man's Search for Meaning: the three-doors gallery.
   Reads the authored .tale cards in the page, builds a one-at-a-time
   gallery with door chips, arrow buttons, and keyboard support.
   No dependencies. Without JS the .tale cards read as a plain list.
   (Cloned from js/imperfection.js; same shell, same pattern.) */
(function () {
  'use strict';
  ...
  var doors = [].slice.call(source.querySelectorAll('.tale')).map(function (el) {
    return {
  ...
  var chips = doors.map(function (d, i) {
  ...
  function render() {
    ...
    prev.disabled = cur === 0;
    next.disabled = cur === doors.length - 1;
  ...
  function go(i) {
    cur = Math.max(0, Math.min(doors.length - 1, i));
    render();
  }
