...
try {
  ...
  await sleep(1400);
  check('claw follows mouse hover without grabbing',await game('bathTool.pointer===null && !bathTool.held && Math.abs(bathTool.x-bathGuests[0].s.x)<4'));
  await press(guestPoint);
  await sleep(300);
  check('quick mouse click catches the actual visitor',await game('bathTool.held===bathGuests[0] && bathGuests[0].manual'));
  ...
  await send('Input.dispatchMouseEvent',{type:'mouseMoved',...point,buttons:0});
  await sleep(1600);
  check('hover carries the guest without a held button',await game('bathTool.pointer===null && bathTool.held===bathGuests[0]'));
  ...
  await send('Input.dispatchMouseEvent',{type:'mouseMoved',...point,buttons:0});
  await sleep(500);
  check('hose follows mouse hover without spending water',await game('bathTool.flow===0 && siphon.tank[0]')===beforeHover);
  await send('Input.dispatchMouseEvent',{type:'mousePressed',...point,button:'left',clickCount:1});
  await sleep(2500);
  ...
  await send('Input.dispatchMouseEvent',{type:'mouseReleased',...point,button:'left',clickCount:1});
  await sleep(500);
  const stopped=await game('siphon.tank[0]');
  point.x+=30;
  await send('Input.dispatchMouseEvent',{type:'mouseMoved',...point,buttons:0});
  await sleep(300);
  check('releasing click stops water while hover keeps aiming',await game('!bathTool.spraying && bathTool.output===0 && siphon.tank[0]')===stopped);
  ...
  const physics=await game(`(function(){var reports=[];
    for(var fps of [30,60,144]){
      ...
      bathToolSelect('claw');bathTool.x=bathTool.tx=s.x;bathTool.y=bathTool.ty=s.y-30;bathToolAction('tool-grip');
  ...
  await sleep(700);
  check('touch drag aims without automatically pouring',await game('bathTool.pointer!==null && bathTool.flow===0 && siphon.tank[0]===20000 && !bathPtrDown'));
  await send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});
  await press(buttonCenter('tool-valve'),true);
  await sleep(600);
  check('mobile Pour button starts water with no held finger',await game('bathTool.pointer===null && bathTool.valve && siphon.tank[0]<20000'));
  await press(buttonCenter('tool-valve'),true);
  await sleep(500);
  check('mobile Stop button closes the hose',await game('!bathTool.valve && bathTool.output===0'));
  await press(buttonCenter('tool-valve'),true);
  await send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{...point,id:8}]});
  await send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[{x:point.x+35,y:point.y+10,id:8}]});
  await sleep(500);
  check('one finger can reposition the running hose',await game('bathTool.pointer!==null && bathTool.valve && bathTool.output>0'));
  ...
  await sleep(1400);
  await send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});
  check('touch positioning does not grab automatically',await game('!bathTool.held && !bathTool.grab && bathToolTouchControls()'));
  await press(buttonCenter('tool-grip'),true); await sleep(400);
  check('mobile Grab button picks up the guest',await game('bathTool.held===bathGuests[0]'));
  await screenshot('phone-touch-claw');
